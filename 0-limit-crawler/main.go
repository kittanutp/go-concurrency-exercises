...
func Crawl(url string, depth int, wg *sync.WaitGroup, throtle <-chan time.Time) {
	defer wg.Done()
	<-throtle
	if depth <= 0 {
		return
	...
	for _, u := range urls {
		...
		// called concurrently
		go Crawl(u, depth-1, wg, throtle)
	}
}

func main() {
	var wg sync.WaitGroup
	throtle := time.Tick(time.Second)
	wg.Add(1)
	Crawl("http://golang.org/", 4, &wg, throtle)
	wg.Wait()
}
